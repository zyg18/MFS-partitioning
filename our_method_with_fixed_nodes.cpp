...
std::tuple<CSRMatrix, std::vector<int>, std::vector<std::vector<int>>, std::vector<int>, std::vector<std::tuple<int, int>>, std::unordered_set<int>> coarsen_graph(
    CSRMatrix& graph,  
...
{   
    ...
    return {new_graph, mapping, remapping, new_node_weight, new_fixed_nodes, new_neighboring_nodes};
}
...
struct CompareFirst { //da ding dui
...
int main(int argc, char* argv[]) {
    ...
    while (coarsen_continue) {
        ...
        auto [new_graph, mapping, remapping, new_node_weight, new_fixed_node, new_neighboring_nodes] = coarsen_graph(current_graph, current_node_weight, current_fixed_nodes, current_neighboring_nodes, int(capacity*0.48)); //omit fixed nodes
